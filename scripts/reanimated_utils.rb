...
def assert_minimal_react_native_version(config)
  ...
  currentVersion = config[:react_native_minor_version]
  
  puts "[Reanimated] Found React Native minor version: #{currentVersion}"
  if currentVersion < minimalReactNativeVersion
